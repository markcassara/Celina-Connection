...
// Helper to perform generateContent with automatic model fallback (e.g. gemini-3.5-flash -> gemini-2.5-flash) and tool stripping on quota limits
async function generateContentWithFallback(ai: GoogleGenAI, params: {
  ...
  try {
    ...
    // Fall back to a more highly-available model if the primary model is busy, or if search tools cause quota errors
    const errMsg = String(error.message || error || "");
    const isSearchError = errMsg.includes("googleSearch") || errMsg.includes("grounding");
    const isOverloadOrQuota = error.status === 503 || error.status === 429 || errMsg.includes("quota") || errMsg.includes("demand") || errMsg.includes("RESOURCE_EXHAUSTED") || errMsg.includes("UNAVAILABLE") || errMsg.includes("limit") || errMsg.includes("exhausted");
...
// Endpoint to check payment configuration status
...
// Endpoint to check AI configuration status
...
// Endpoint for AI search insights
...
2. insights: A friendly, conversational paragraph (in Markdown) explaining why these businesses are recommended for their search. Use the real names of the businesses. Be brief, warm, and helpful.`;
    ...
    res.status(500).json({ error: error.message || "Failed to process AI search query" });
  }
});

// Endpoint for AI chat assistant with search grounding
...
5. If search grounding provides source links, you do not need to list them as plain text at the bottom. We will extract the grounding chunks automatically and show them. Just mention what you found nicely!`;
    ...
    // Ensure the conversation history starts with a user message for Gemini
...
app.post("/api/create-checkout-session", async (req, res) => {
  try {
    ...
    const appUrl = process.env.APP_URL || `http://${req.headers.host}` || "http://localhost:3000";
    ...
    // 1. Add Main Subscription Plan Line Item
    if (tier === "premium") {
      if (interval === "year") {
        if (priceIdPremiumAnnual) {
          line_items.push({
            price: priceIdPremiumAnnual,
            quantity: 1,
          });
        } else {
          line_items.push({
        ...
        if (priceIdPremiumMonthly) {
          line_items.push({
            price: priceIdPremiumMonthly,
            quantity: 1,
          });
        } else {
          line_items.push({
      ...
      if (interval === "year") {
        ...
        if (priceIdProMonthly) {
          line_items.push({
            price: priceIdProMonthly,
            quantity: 1,
          });
        } else {
          line_items.push({
    ...
    // 2. Add Additional Business Listings Add-on Line Item if requested
    if (addonQuantity && addonQuantity > 0) {
      if (interval === "year") {
        if (priceIdAddonAnnual) {
          line_items.push({
            price: priceIdAddonAnnual,
          ...
          line_items.push({
            price_data: {
              ...
              unit_amount: 3600, // $36.00/year per additional business
              recurring: {
                interval: "year",
              },
            },
        ...
        if (priceIdAddonMonthly) {
          line_items.push({
            price: priceIdAddonMonthly,
            quantity: addonQuantity,
          });
        } else {
          line_items.push({
            price_data: {
              ...
              unit_amount: 400, // $4.00/month per additional business
              recurring: {
                interval: "month",
              },
            },
