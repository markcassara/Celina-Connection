...
<body>
  ...
  <div class="summary"><strong>Total signatures:</strong> ${signatures.length}<br />Purpose: neighbor/stakeholder petition contact record for Legacy Hills submissions collected through Celina Connection.</div>
  <table>
    <thead><tr><th>#</th><th>Name</th><th>Street Address</th><th>Phone</th><th>Email</th><th>Signed</th><th>Signature</th></tr></thead>
    <tbody>${rows || '<tr><td colspan="7">No signatures collected yet.</td></tr>'}</tbody>
  </table>
</body>
</html>`;
  };
  ...
  app.get("/api/admin/petitions/legacy-hills/export.csv", requireAdminToken, async (_req, res) => {
    const signatures = await repository.listLegacyHillsPetitionSignatures();
    const headers = ['Signed At', 'First Name', 'Last Name', 'Email', 'Phone', 'Street Address', 'Neighborhood', 'Comments', 'GHL Contact ID'];
